<template v-slot="links">
  <div class="sidebar" :data="backgroundColor">
    <div class="sidebar-wrapper" id="style-3">
      <div class="logo">
        <a
          href="https://github.com/pedroporo"
        ...
        <a href="https://github.com/pedroporo" class="simple-text logo-normal">
          {{ title }}
        </a>
      </div>

      <ul class="nav">
        <slot is="ul" name="links">
          <slot></slot>
          <SidebarLink
...
export default {
  ...
  props: {
    ...
    backgroundColor: { type: String, default: "vue" },
    activeColor: {
      type: String,
      default: "success",
      validator: (value) =>
  ...
  methods: {
    /*findActiveLink() {
      this.links.forEach((link, index) => {
        if (link && typeof link.isActive === "function" && link.isActive()) {
    ...
    updateLinksFromRefs() {
      this.links = Object.keys(this.$refs)
        .filter((refName) => refName.startsWith("sidebarLink"))
        .map((refName) => this.$refs[refName])
        .flat();
    },
    findActiveLink() {
      //this.updateLinksFromRefs();
      this.links.forEach((link, index) => {
        if (link.isActive()) {
          this.activeLinkIndex = index;
        }
    ...
    addLink(link) {
      //console.log(link.rname);
      const sl = this.$router.getRoutes();
      //const test = sl.findIndex((l) => l.path == link.rname);
      //console.log(test);

      //console.log(this.$router.getRoutes());
      //console.log(this.$sidebar);
      const index = sl.findIndex((l) => l.path == link.rname);
